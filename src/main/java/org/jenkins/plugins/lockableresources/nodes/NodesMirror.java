...
public class NodesMirror extends ComputerListener {
  ...
  private static void deleteNotExistingNodes() {
    ...
    while (resourceIterator.hasNext()) {
      ...
      if (resource.isFree()) {
        ...
        LOGGER.config(
            "lockable-resources-plugin: remove node resource '" + resource.getName() + "'.");
        resourceIterator.remove();
      } else {
        LOGGER.warning(
            "lockable-resources-plugin: can not remove node-resource '"
                + resource.getName()
  ...
  private static void mirrorNode(Node node) {
    ...
    if (!exist) {
      ...
      LOGGER.config(
          "lockable-resources-plugin: Node-resource '"
