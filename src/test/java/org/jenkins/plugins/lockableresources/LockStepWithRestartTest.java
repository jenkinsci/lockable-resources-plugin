  ...
  public void lockOrderRestart() throws Throwable {
    sessions.then(
      j -> {
        ...
        j.waitForMessage(
          "[resource1] is locked by " + b1.getFullDisplayName(), b2);
        isPaused(b2, 1, 1);
        WorkflowRun b3 = p.scheduleBuild2(0).waitForStart();
        // Both 2 and 3 are waiting for locking resource1

        j.waitForMessage(
          "[resource1] is locked by " + b1.getFullDisplayName(), b3);
        isPaused(b3, 1, 1);
      });
      ...
      j -> {
        ...
        p.setDefinition(
          new CpsFlowDefinition(
            "lock('resource1') {\n" + "  echo 'inside'\n" + "}\n" + "echo 'Finish'", true));
        WorkflowRun b1 = p.scheduleBuild2(0).waitForStart();
        j.waitForMessage("[resource1] is not free , waiting for execution...", b1);
        isPaused(b1, 1, 1);
